...
with st.sidebar:
    ...
    #start date & end date from date_input
...
sum_orders_items_df = create_sum_order_items_df(main_df)
...
with col2:
    ...
    sns.barplot(
        y="customer_count",
        x="age_group",
        data=byage_df.sort_values(by="customer_count", ascending=False),
        palette=colors,
        ax=ax
    )
...
with col1:
    avg_currency = format_currency(rfm_df.monetary.mean(), "AUD", locale='es_CO')
    st.metric("Average Currency (days)", value=avg_currency)
